...
def create_property(db: Session, property_data: PropertyCreate):
    ...
    # Si le statut n'est pas fourni, le définir par défaut
...
def get_properties(
    ...
    # Filtres par type
    if filters.type:
        query = query.filter(Property.type.in_(filters.type))
    
    # Filtres par statut
    if filters.status:
        query = query.filter(Property.status.in_(filters.status))
